...
function getValueIcon(
  ...
  if (
    Icon &&
    (featureIdentifier === 'private_cloud_deployment' ||
      featureIdentifier === 'sdk_coverage')
  ) {
                ...
                <span className="bg-lime-100 text-lime-600 w-full min-h-6 flex flex-wrap gap-1 items-center justify-center rounded p-1">
                  {values && values.length > 0 ? (
                    values.map((v) => (
                      <span
                        key={v}
                        className="text-xs px-1 py-0.5 bg-lime-200 rounded flex items-center gap-1"
                      >
                        {getValueIcon(featureIdentifier, v)}
                        {v}
                      </span>
                    ))
                  ) : (
                ...
                <span className="bg-red-100 text-red-600 w-full min-h-6 flex flex-wrap gap-1 items-center justify-center rounded p-1">
                  {values && values.length > 0 ? (
                    values.map((v) => (
                      <span
                        key={v}
                        className="text-xs px-1 py-0.5 bg-red-200 rounded flex items-center gap-1"
                      >
                        {getValueIcon(featureIdentifier, v)}
                        {v}
                      </span>
                ...
                <span className="bg-yellow-100 text-yellow-600 w-full min-h-6 flex flex-wrap gap-1 items-center justify-center rounded p-1">
                  {values && values.length > 0 ? (
                    values.map((v) => (
                      <span
                        key={v}
                        className="text-xs px-1 py-0.5 bg-yellow-200 rounded flex items-center gap-1"
                      >
                        {getValueIcon(featureIdentifier, v)}
                        {v}
                      </span>
                ...
                <span className="bg-blue-100 text-blue-600 w-full min-h-6 flex flex-wrap gap-1 items-center justify-center rounded p-1">
                  {values && values.length > 0 ? (
                    values.map((v) => (
                      <span
                        key={v}
                        className="text-xs px-1 py-0.5 bg-blue-200 rounded flex items-center gap-1"
                      >
                        {getValueIcon(featureIdentifier, v)}
                        {v}
                      </span>
                ...
                <span className="bg-gray-100 text-gray-600 w-full min-h-6 flex flex-wrap gap-1 items-center justify-center rounded p-1">
                  {values && values.length > 0 ? (
                    values.map((v) => (
                      <span
                        key={v}
                        className="text-xs px-1 py-0.5 bg-gray-200 rounded flex items-center gap-1"
                      >
                        {getValueIcon(featureIdentifier, v)}
                        {v}
                      </span>
