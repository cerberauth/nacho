...
export async function generateOpenIDProviderVsMetadata(
  lang: Locale,
  aId: string,
  bId: string,
): Promise<Metadata> {
...
export async function OpenIDProviderVsPage({
  lang,
  aId,
  bId,
}: {
  ...
  const iamProviderA = iamProviders.find(
    (p) => p.identifier === providerA.identifier,
  )
  const iamProviderB = iamProviders.find(
    (p) => p.identifier === providerB.identifier,
  )
  const iamVsOrder =
    iamProviderA && iamProviderB
        ...
        {iamVsOrder ? (
          <>
            ...
            <Link
              href={langUrl(
                lang,
                `/iam/providers/${iamVsOrder[0]}/vs/${iamVsOrder[1]}`,
              )}
