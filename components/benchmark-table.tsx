    ...
    <div className="flex flex-col">
      <div className="flex gap-1">
        <div
          ...
          className="sticky left-0 z-10 bg-white shrink-0 w-[280px] min-w-[280px] px-2 pt-4 pb-1"
        >
          <a href={`#${category.name.trim()}`}>
            <h2 className="text-xl font-mono text-slate-950 before:content-['#'] before:hidden before:absolute before:-left-4 hover:before:block">
              {category.name}
            </h2>
          </a>
        </div>
      ...
      <ul className="flex flex-col gap-1">
        {category.rows.map((row) => (
          <li
            key={`category-row-${row.identifier}`}
          ...
          >
            ...
            {row.cells.map((cell) => (
              <div
                key={`row-${row.identifier}-cell-${cell.identifier}`}
                className={`min-w-[124px] w-full flex items-start justify-center transition-opacity ${dimmedProviders?.has(cell.identifier) ? 'opacity-25' : ''}`}
              >
                <StatusCell
