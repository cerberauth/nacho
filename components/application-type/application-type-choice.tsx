...
export function ApplicationTypeChoice({
  withUserInteraction,
  onChange,
}: ApplicationTypeGridProps) {
  return (
    <>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {applicationTypes
          .filter(
            (option) => option.withUserInteraction === withUserInteraction,
          )
          .map((option) => (
            <ChoiceCard
              key={option.value}
...
export function MiniApplicationTypeChosenCard({
  className,
  applicationType,
  onClick,
}: MiniApplicationTypeChosenCardProps) {
  const option = useMemo(
    () => applicationTypes.find((option) => option.value === applicationType),
    [applicationType],
  )
  if (!option) {
    return null
  ...
  return (
    <MiniChoiceCard
      className={className}
      icon={<ApplicationTypeIcon type={applicationType} />}
      title={option.label || ''}
      onClick={onClick}
    />
  )
